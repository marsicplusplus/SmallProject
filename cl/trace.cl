...
//&& v != RED
// Zero test represents the thing we want to exclude
#define BRICKSTEP(exitLabel, zeroTest)															\
	v = o + (p >> 20) + ((p >> 7) & (BMSK * BRICKDIM)) + (p & BMSK) * BDIM2;            \
	v = brick0[v]; if (zeroTest)   { *dist = t + to, * side = last; return v; }		\
...
#define BRICKSTEP(exitLabel, zeroTest)															\
	...
	if (zeroTest) { *dist = t + to, * side = last; return v; }									\
	t = min( tm.x, min( tm.y, tm.z ) ), last = 0;										\
	if (t == tm.x) tm.x += td.x, p += dx;												\
	if (t == tm.y) tm.y += td.y, p += dy, last = 1;										\
	if (t == tm.z) tm.z += td.z, p += dz, last = 2;										\
...
uint TraceThrough(uint toTraceThrough, uint compareMask, float4 A, const float4 B, float* dist, uint* side, __read_only image3d_t grid,
	...
	int steps)
{
	...
	while (1)
	{
		...
		if (o != 0 || toTraceThrough != 0)
		{
			...
			#endif
			#ifdef ISAMPERE
				GRIDSTEP_TRACETHROUGH_IDENTICAL(exit1, toTraceThrough, compareMask); GRIDSTEP_TRACETHROUGH_IDENTICAL(exit2, toTraceThrough, compareMask); // Ampere has massive L1I$;
				GRIDSTEP_TRACETHROUGH_IDENTICAL(exit3, toTraceThrough, compareMask); GRIDSTEP_TRACETHROUGH_IDENTICAL(exit4, toTraceThrough, compareMask); // unroll 4x for best performance.
			#else
...
// Trace a ray that only hits opaque surfaces, traveling through translucent ones
uint TraceOpaqueRay( float4 A, const float4 B, float* dist, uint* side, __read_only image3d_t grid,
	__global const unsigned char* uberGrid,
	__global const PAYLOAD* brick0,
#if ONEBRICKBUFFER == 0
	__global const PAYLOAD* brick1,
	...
	int steps)
{
	...
	while (1)
	{
		...
		if (o)
		{
			...
			#endif
			#ifdef ISAMPERE
				GRIDSTEP_OPAQUE(exit1); GRIDSTEP_OPAQUE(exit2); // Ampere has massive L1I$;
				GRIDSTEP_OPAQUE(exit3); GRIDSTEP_OPAQUE(exit4); // unroll 4x for best performance.
			#else
...
)
{
	...
	while (1)
	{
		...
		if (o)
		{
			...
			#endif
			#ifdef ISAMPERE
				GRIDSTEP_ALL(exit1); GRIDSTEP_ALL(exit2); // Ampere has massive L1I$;
				GRIDSTEP_ALL(exit3); GRIDSTEP_ALL(exit4); // unroll 4x for best performance.
			#else
