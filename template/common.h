...
struct RenderParams
{
	...
	int accumulate, spatial, temporal, skyDomeSampling, editorEnabled, drawGrid; //booleans
	uint numberOfLights;
...
#define GRIDSIZE	(GRIDWIDTH * GRIDHEIGHT * GRIDDEPTH)
...
inline bool IsEmitter(const uint v)
{
	return (v >> 16 & 255) > 0;
}
